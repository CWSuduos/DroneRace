...
def main(args=None):
    ...
    # Список шагов для выполнения маршрута
    steps = [
        # Начало маршрута: первый круг
        Ascend(2, 0.7),  # Подъем на 2 метра с допустимым отклонением 0.7 метра
        Navigate(248.3, 244.8, 196.3, 1.22, 0.7),  # Навигация к первой точке с углом и допустимым отклонением
        GatePassageController(0, 1, 0.8),  # Прохождение через ворота (индекс 0, цель 1, отклонение 0.8)
        
        # Продолжение маршрута
        Navigate(243.3, 254.0, 194.9, 1.57),  # Навигация ко второй точке
        GatePassageController(1, 2, 0.8),  # Прохождение через ворота (индекс 1, цель 2)
        
        Navigate(249.0, 258.8, 194.7, 5.60),  # Перемещение к третьей точке
        GatePassageController(2, 2, 1.0),  # Прохождение через ворота (индекс 2, цель 2)
        
        Navigate(254.1, 248.5, 196.9, 4.73),  # Перемещение к четвертой точке
        GatePassageController(3, 3, 1.0),  # Прохождение через ворота (индекс 3, цель 3)
        
        Navigate(254.1, 241.1, 195.3, 1.57),  # Перемещение к пятой точке
        GatePassageController(4, 3, 0.8),  # Прохождение через ворота (индекс 4, цель 3)
        
        Navigate(253.8, 252.2, 194.9, 2.80),  # Перемещение к шестой точке
        GatePassageController(5, 3.4, 0.7),  # Прохождение через ворота (индекс 5, цель 3.4)
        
        Navigate(240.0, 249.8, 194.9, 5.00),  # Перемещение к седьмой точке
        GatePassageController(6, 2, 0.8),  # Прохождение через ворота (индекс 6, цель 2)

        # Второй круг: повторение маршрута
        Navigate(248.3, 244.8, 196.3, 1.22, 0.7),
        GatePassageController(0, 1, 0.8),
        
        Navigate(243.3, 254.0, 194.9, 1.57),
        GatePassageController(1, 2, 0.8),
        
        Navigate(249.0, 258.8, 194.7, 5.60),
        GatePassageController(2, 2, 1.0),
        
        Navigate(254.1, 248.5, 196.9, 4.73),
        GatePassageController(3, 3, 1.0),
        
        Navigate(254.1, 241.1, 195.3, 1.57),
        GatePassageController(4, 3, 0.8),
        
        Navigate(253.8, 252.2, 194.9, 2.80),
        GatePassageController(5, 3.4, 0.7),
        
        Navigate(240.0, 249.8, 194.9, 5.00),
        GatePassageController(6, 2, 0.8),

        # Третий круг: завершение маршрута
        Navigate(248.3, 244.8, 196.3, 1.22, 0.7),
        GatePassageController(0, 1, 0.8),
    ...
    controller = SimpleController(node, steps)
